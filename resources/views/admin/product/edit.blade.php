<x-admin-layout>
  ...
  <x-slot name="header">
      <nav class="breadcrumb pd-0 mg-0 tx-12">
          ...
          <a class="breadcrumb-item" href="{{ route('product.index') }}">إدارة المنتجات</a>
          <span class="breadcrumb-item active">{{ $product->name_ar }}</span>
  ...
  <div class="card pd-25">
    ...
    <form action="{{ route('product.update', ['product' => $product->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row mg-b-25">
              <div class="col-lg-6">
                <div class="form-group">
                  ...
                  <input class="form-control" type="text" name="name_ar" value="{{ $product->name_ar }}" placeholder="ادخل عنوان المنتج بالعربي">
                </div>
              </div><!-- col-4 -->
              <div class="col-lg-6">
                <div class="form-group">
                  <label class="form-control-label">العنوان بالانجليزي: <span class="tx-danger">*</span></label>
                  <input class="form-control" type="text" name="name" value="{{ $product->name }}" placeholder="ادخل عنوان المنتج بالانجليزي">
                </div>
              </div><!-- col-4 -->
              <div class="col-lg-12">
                <div class="form-groub">
                  <label for="">صورة المنتج <span class="tx-danger">*</span></label>
                  <img style="width: 200px;height: 200px;display: block" src="{{ asset($product->photo) }}" class="img-fluid img-thumbnail" alt="">
                </div>
                <div class="form-group">
                  <input id="customFile" class="custom-file-input" type="file" name="photo" value="{{ $product->photo }}">
                  <label style="top: 213px;width: 200px;" class="custom-file-label m-3" for="customFile"></label>
                </div>
              </div><!-- col-4 -->
              <div class="col-lg-3 mg-t-20 mg-lg-t-0">
                <label class="ckbox">
                  <input type="checkbox" name="is_featured" {{ $product->is_featured ? 'checked' : ''}} value="1"><span>المنتج مميز ؟ <small>المنتجات المميزة سيتم عرضها على الصفحة الرئيسية</small></span>
                </label>
              </div><!-- col-8 -->
            </div>
          </div><!-- tab-pane -->
            ...
            <div class="row mg-b-25">
              <div class="col-lg-12">
                <div class="form-group mg-b-10-force">
                  ...
                  <textarea class="form-control summernote" type="text" name="descr_ar" placeholder="ادخل الوصف بالعربي">{{ $product->descr_ar }}</textarea>
                </div>
              </div><!-- col-8 -->
              <div class="col-lg-12">
                <div class="form-group mg-b-10-force">
                  <label class="form-control-label">الوصف بالانجليزي: <span class="tx-danger">*</span></label>
                  <textarea class="form-control summernote" type="text" name="descr" placeholder="ادخل الوصف بالانجليزي">{{ $product->descr }}</textarea>
                </div>
              </div><!-- col-8 -->
            </div>
          </div><!-- tab-pane -->
      ...
      <div class="card-footer mt-2">
        ...
        <a href="{{ route('product.index') }}" class="btn btn-secondary">الغاء</a>
